...
class KDTree
{
  ...
  public:
     ...
     *  Point::operator<().
     */
    // vector<double> getDistance(const Point<Dim>& target, const Point<Dim>& currentBest, const Point<Dim>& potential);
  ...
  private:
     ...
     */
    unsigned partition(vector<Point<Dim>>& list, unsigned left, unsigned right, unsigned pivotIndex, int curDim);
    void swap(vector<Point<Dim>>& list, unsigned index_1, unsigned index_2);
    Point<Dim>& select(vector<Point<Dim>>& list, unsigned left, unsigned right, int k, int curDim);
    KDTreeNode* buildTree(vector<Point<Dim>>& points, int dim, unsigned left, unsigned right);

    Point<Dim> Neighbor_helper(const Point<Dim>& query, int dim, KDTreeNode* root) const;
